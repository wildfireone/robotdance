...
if(req.query.port || getPort(req.query.port)){
  ...
  var motortime = 1000;
  if(req.query.time){motortime = parseInt(req.query.time)}
  if (!motor.connected)
...
var getStop = function(stopCommand){
  switch(stopCommand){
    case "break": return motor.stopActionValues.break;
    case "coast": return motor.stopActionValues.coast;
    case "hold": return motor.stopActionValues.hold;
  }
}
